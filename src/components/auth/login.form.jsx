...
import { authConfig, checkPasswordStrength } from './auth.config'
...
export default function LoginForm({borderless, className}) {
  ...
  return (
    <Card className={`w-full max-w-md ${borderless && "border-none shadow-none bg-background" } max-sm:border-none  max-sm:shadow-none  max-sm:bg-background mx-auto ${className && className}`}>
        <CardHeader className="space-y-1">
          <Link href={"/"} className='mx-auto mb-4 mt-2'>
            <Image height={40} width={40} src={authConfig.content.logo} alt="icon" /> 
          </Link>
          <CardTitle className="text-2xl font-bold text-center">Login to {authConfig.content.name}</CardTitle>
